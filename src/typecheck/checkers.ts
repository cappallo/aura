...
export function checkProperty(
  ...
  const state = createBaseInferState(ctx, errors, syntheticFn, filePath);
...
function verifyDeclaredEffects(
  fn: ast.FnDecl,
  ctx: TypecheckContext,
  errors: TypeCheckError[],
  filePath?: string,
...
function createBaseInferState(
  ...
  const state: InferState = {
    nextTypeVarId: 0,
    substitutions: new Map<number, Type>(),
    errors,
    ctx,
    currentFunction,
    expectedReturnType: UNIT_TYPE,
